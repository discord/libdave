...
std::unique_ptr<MlsKeyRatchet> MlsKeyRatchetFromJS(val keyRatchet) {
    ...
    auto cipherSuite = ::mlspp::CipherSuite(static_cast<::mlspp::CipherSuite::ID>(keyRatchet["cipherSuite"].as<uint16_t>()));
...
public:
    SessionWrapper(std::string ctx, std::string authSessionId, val callback) {
        session_ = std::make_unique<Session>(ctx.c_str(), authSessionId, [callback](std::string source, std::string message) {
            callback(source, message);
        });
    }

    void Init(ProtocolVersion version, uint64_t groupId, std::string const& selfUserId, std::shared_ptr<::mlspp::SignaturePrivateKey>& transientKey) {
    ...
    void SetProtocolVersion(ProtocolVersion version) {
        session_->SetProtocolVersion(version);
    }   
    ...
    void SetExternalSender(val externalSender) {
        if (!externalSender.isNull()) {
            std::vector<uint8_t> externalSenderVec = emscripten::convertJSArrayToNumberVector<uint8_t>(externalSender);
    ...
    val ProcessProposals(val proposals, val recognizedUserIDs) {
        std::vector<uint8_t> proposalsVec = emscripten::convertJSArrayToNumberVector<uint8_t>(proposals);
        auto recognizedUserIDsVec = emscripten::vecFromJSArray<std::string>(recognizedUserIDs);
        auto recognizedUserIDsSet = std::set<std::string>(recognizedUserIDsVec.begin(), recognizedUserIDsVec.end());
    ...
    val GetMarshalledKeyPackage() {
        return ToOwnedTypedArray(session_->GetMarshalledKeyPackage());
    }
...
public:
    ...
    ProtocolVersion GetProtocolVersion() {
        return encryptor_->GetProtocolVersion();
    }
    ...
    size_t Encrypt(MediaType mediaType, uint32_t ssrc, int framePtr, size_t frameLength, size_t frameCapacity) {
        ...
        auto result = encryptor_->Encrypt(mediaType, ssrc, frameView, encryptedFrameView, &bytesWritten);
    ...
    void SetProtocolVersionChangedCallback(val callback) {
        encryptor_->SetProtocolVersionChangedCallback([callback]() {
            callback();
        });
    }
...
public:
    DecryptorWrapper() {
        decryptor_ = std::make_unique<Decryptor>();
    }
...
EMSCRIPTEN_BINDINGS(dave) { 
    ...
    class_<discord::dave::mls::TransientKeys>("TransientKeys")
        .constructor<>()
        .function("GetTransientPrivateKey", &discord::dave::mls::TransientKeys::GetTransientPrivateKey)
    ...
    class_<discord::dave::mls::SessionWrapper>("Session")
        ...
        .function("GetLastEpochAuthenticator", &discord::dave::mls::SessionWrapper::GetLastEpochAuthenticator)
        ...
        .function("GetMarshalledKeyPackage", &discord::dave::mls::SessionWrapper::GetMarshalledKeyPackage)
    ...
    class_<discord::dave::EncryptorWrapper>("Encryptor")
        ...
        .function("GetMaxCiphertextByteSize", &discord::dave::EncryptorWrapper::GetMaxCiphertextByteSize)
        .function("Encrypt", &discord::dave::EncryptorWrapper::Encrypt, emscripten::allow_raw_pointers())
        .function("SetProtocolVersionChangedCallback", &discord::dave::EncryptorWrapper::SetProtocolVersionChangedCallback);
    ...
    class_<discord::dave::DecryptorWrapper>("Decryptor")
        ...
        .function("TransitionToPassthroughMode", &discord::dave::DecryptorWrapper::TransitionToPassthroughMode)
        .function("GetMaxPlaintextByteSize", &discord::dave::DecryptorWrapper::GetMaxPlaintextByteSize)
        .function("Decrypt", &discord::dave::DecryptorWrapper::Decrypt, emscripten::allow_raw_pointers());
